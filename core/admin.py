...
from .models import (
    Skill, ProjectCategory, Project, Experience, 
    ContactMessage, PortfolioSettings
)
...
class ExperienceAdmin(admin.ModelAdmin):
    list_display = ('title', 'company_or_institution', 'experience_type', 'duration', 'is_current', 'is_featured')
...
class PortfolioSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Información Personal', {
            'fields': ('site_title', 'tagline', 'about_me')
        }),
        ('Contacto y Redes', {
            'fields': ('email', 'github_username', 'linkedin_url')
        }),
        ('Archivos', {
            'fields': ('cv_file_path',)
        }),
